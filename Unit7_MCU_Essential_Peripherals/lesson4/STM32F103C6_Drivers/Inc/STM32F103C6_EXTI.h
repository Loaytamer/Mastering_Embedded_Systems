...
typedef struct{
	EXTI_GPIO_Mapping_t EXTI_PIN; 		//spcifies the external GPIO mapping
								 	 	// this parameter must be set based on @ref EXTI_define
	uint8_t				Trigger_Case;	//Specifies rising or falling or any change
										// this parameter must be set based on @ref EXTI_Trigger_define
	uint8_t				IRQ_EN;			// Enable or Disable the EXTI(That will Enable the IRQ MASK in EXTI and also on the NVIC INT Controller)
